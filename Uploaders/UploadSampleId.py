...
class UploadSampleId:
    ...
    def uploadBatch(self):
        ...
        if self.ableToOpenFile:
            try:
                ...
                UnableToParseFile(self.sampleIdReader.fileName)
        else:
    ...
    def updateRow(self):
        ...
        sqlUpdate = "UPDATE sort_chems SET site_id = ?," \
                    " project_id = ?, date_sampled = ?, time_sampled = ?," \
                    " ph = ?, orp = ?, o2_percent = ?, o2_mg = ?, conductance = ?," \
                    " temperature = ?, pressure = ?, calibrated = ?, q_salt_grams = ?," \
                    " q_time = ?, notes = ?, conditions = ?, samplers = ?, volume_filtered = ?, " \
                    "aqualog_yes = ?, doc_isotopes_yes = ?, elementar_yes = ?, scan_yes = ?, ic_yes = ?, " \
                    "icp_yes = ?, lachat_yes = ?, no3_isotopes_yes = ?, srp_yes = ?, water_isotopes_yes = ?, " \
                    " ignore_yes = ?, datetime_uploaded = ?, file_path = ? WHERE sort_chem = ?;"
        updateTuple = (self.sampleIdReader.site, self.sampleIdReader.project, self.sampleIdReader.date,
                       self.sampleIdReader.time, self.sampleIdReader.ph, self.sampleIdReader.orp,
                       self.sampleIdReader.o2percent, self.sampleIdReader.o2mg, self.sampleIdReader.cond,
                       self.sampleIdReader.temp, self.sampleIdReader.press, self.sampleIdReader.cal,
                       self.sampleIdReader.qsalt, self.sampleIdReader.qtime,
                       self.sampleIdReader.notes, self.sampleIdReader.conditions, self.sampleIdReader.samplers,
                       self.sampleIdReader.volumeFiltered, self.sampleIdReader.aqualog, self.sampleIdReader.doc,
    ...
    def uploadRow(self):
        sqlInsert = "INSERT INTO sort_chems (site_id, project_id, date_sampled," \
                    "time_sampled, sort_chem, ph, orp, o2_percent, o2_mg, conductance, temperature," \
                    "pressure, calibrated, q_salt_grams, q_time, notes, conditions, samplers, volume_filtered, " \
                    "aqualog_yes, doc_isotopes_yes,elementar_yes, scan_yes, ic_yes, icp_yes, lachat_yes, " \
                    "no3_isotopes_yes, srp_yes, water_isotopes_yes, ignore_yes, datetime_uploaded, file_path) " \
                    "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
        insertTuple = (self.sampleIdReader.site, self.sampleIdReader.project, self.sampleIdReader.date,
                       self.sampleIdReader.time, self.sampleIdReader.sortChem, self.sampleIdReader.ph,
                       self.sampleIdReader.orp, self.sampleIdReader.o2percent, self.sampleIdReader.o2mg,
                       self.sampleIdReader.cond, self.sampleIdReader.temp, self.sampleIdReader.press,
                       self.sampleIdReader.cal, self.sampleIdReader.qsalt, self.sampleIdReader.qtime,
                       self.sampleIdReader.notes, self.sampleIdReader.conditions, self.sampleIdReader.samplers,
                       self.sampleIdReader.volumeFiltered, self.sampleIdReader.aqualog,
                       self.sampleIdReader.doc, self.sampleIdReader.elementar, self.sampleIdReader.scan,
                       self.sampleIdReader.ic, self.sampleIdReader.icp, self.sampleIdReader.lachat,
                       self.sampleIdReader.no3, self.sampleIdReader.srp, self.sampleIdReader.water,
                       self.sampleIdReader.ignore, self.sampleIdReader.datetimeUploaded, self.sampleIdReader.filePath)
